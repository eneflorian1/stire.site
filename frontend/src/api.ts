...
export async function fetchArticleDetailByPath(category: string, dateStr: string, slug: string): Promise<ArticleDetail> {
  // Directly call SEO JSON route mirrored by backend
  return httpGet<ArticleDetail>(`/${category}/articol/${dateStr}/${slug}`);
}
