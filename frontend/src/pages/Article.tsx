...
import { fetchArticleDetail, fetchArticleDetailByPath } from '../api';
...
export default function ArticlePage() {
  const { id: slugOrId, category, date, slug } = useParams();
  ...
  useEffect(() => {
    // Prefer new SEO path if params exist
    if (category && date && slug) {
      setLoading(true);
      fetchArticleDetailByPath(category, date, slug)
        .then(setArticle)
