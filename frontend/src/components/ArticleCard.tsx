...
function slugify(title: string): string {
  // Normalize and remove diacritics, then slug rules
...
export function ArticleCard({ article, variant }: { article: Article; variant?: 'featured' | 'banner' }) {
  ...
  const catSlug = slugify(article.category || 'stiri');
  const d = new Date(article.published_at);
  const dd = String(d.getDate()).padStart(2, '0');
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  const yyyy = String(d.getFullYear());
  const dateStr = `${dd}-${mm}-${yyyy}`;
  // New SEO path (no id in URL)
  const href = `/${catSlug}/articol/${dateStr}/${slug}`;
