...
if [ -f "$APP_DIR/ops/systemd/stirix.service" ] && command -v systemctl >/dev/null 2>&1; then
  ...
  # Inject indexing-related environment variables if provided (for Google ping + GitHub dispatch)
  # These should typically be passed from GitHub Actions as env vars when calling this script.
  sed -i "s|__PUBLIC_SITE_URL__|${PUBLIC_SITE_URL:-https://stire.site}|g" "$TMP_SERVICE"
  sed -i "s|__GITHUB_INDEXING_REPO__|${GITHUB_INDEXING_REPO:-}|g" "$TMP_SERVICE"
  sed -i "s|__GITHUB_INDEXING_TOKEN__|${GITHUB_INDEXING_TOKEN:-}|g" "$TMP_SERVICE"
